...
async function checkNonce(nonce) {
  ...
  // Create the table on first use so the deploy needs no out-of-band setup step
  // (our access is scoped to code deploy; we can't pre-create storage tables).
  try { await table.createTable(); } catch (e) { /* already exists */ }
  try {
